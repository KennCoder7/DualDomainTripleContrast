...
class Processor(IO):
    ...
    def start(self):
        ...
        if self.arg.phase == 'train':
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                # save model
                if self.arg.bool_save_model and ((epoch + 1) % self.arg.save_interval == 0) or (
        ...
        # extract feature phase
        elif self.arg.phase == 'extract_feature':
            # the path of weights must be appointed
            if self.arg.weights is None:
                raise ValueError('Please appoint --weights.')
            self.io.print_log('Model:   {}.'.format(self.arg.model))
            ...
            # evaluation
            self.io.print_log('Extract Feature Start:')
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--bool_save_model', type=str2bool, default=True)
